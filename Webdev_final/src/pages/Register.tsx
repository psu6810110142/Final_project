...
const RegisterPage: React.FC = () => {
  ...
  React.useEffect(() => {
    api.get('/levels').then(r => setLevels(r.data)).catch(err => console.error('Fetch levels error:', err));
  }, []);
      ...
      <nav className="navbar">
        <div className="container navbar-container">
          ...
          <div className="navbar-menu">
            <Link to="/" className="menu-item"><Home size={18} /> กลับหน้าหลัก</Link>
            <Link to="/login" className="menu-item active"><LogIn size={18} /> เข้าสู่ระบบ</Link>
          </div>
        </div>
      </nav>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div className="form-group">
                ...
                <input name="username" type="text" className="form-input" placeholder="เช่น somchai123"
                  value={formData.username} onChange={handleChange} required />
              </div>
              <div className="form-group">
                <label className="form-label">อีเมล (Email) *</label>
                <input name="email" type="email" className="form-input" placeholder="name@example.com"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div className="form-group">
                ...
                <input name="password_hash" type="password" className="form-input" placeholder="ขั้นต่ำ 6 ตัวอักษร"
                  value={formData.password_hash} onChange={handleChange} required />
              </div>
              <div className="form-group">
                <label className="form-label">ยืนยันรหัสผ่าน *</label>
                <input name="confirmPassword" type="password" className="form-input" placeholder="กรอกรหัสผ่านอีกครั้ง"
            ...
            <div className="form-group">
              ...
              <input name="full_name" type="text" className="form-input" placeholder="กรอกชื่อและนามสกุลจริง"
                value={formData.full_name} onChange={handleChange} required />
            </div>

            <div className="form-group">
              <label className="form-label">เบอร์โทรศัพท์ (Phone)</label>
              <input name="phone" type="tel" className="form-input" placeholder="เช่น 0812345678"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div className="form-group">
                ...
                <select name="level_id" className="form-input" style={{ cursor: 'pointer' }}
                  value={formData.level_id} onChange={handleChange}>
              ...
              <div className="form-group">
                ...
                <select name="interesting_subject" className="form-input" style={{ cursor: 'pointer' }}
                  value={formData.interesting_subject} onChange={handleChange}>
            ...
            <button type="submit" className="btn-submit"
              style={{ marginTop: '20px', width: '100%', border: 'none', cursor: loading ? 'not-allowed' : 'pointer' }}
              disabled={loading}>
              {loading ? 'กำลังบันทึกข้อมูล...' : 'ลงทะเบียน'}
            </button>
          </form>

          <div className="divider"><span>หรือสมัครด้วย</span></div>

          <button className="btn-google" onClick={() => window.location.href = 'https://wd10.pupasoft.com/api/auth/google'}>
